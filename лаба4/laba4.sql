...
/*1.Напишите запрос с подзапросом для получения данных обо всех оценках студента с фамилией «Иванов». Предположим, что его персональный номер неизвестен*/
SELECT * FROM EXAM_MARKS WHERE STUDENT_ID=(SELECT STUDENT_ID FROM STUDENT where SURNAME='Зайцева' and FNAME='Ольга' );
/*2.Напишите запрос, выбирающий данные об именах всех студентов, имеющих по предмету с идентификатором 22 балл выше общего среднего балла*/
...
/*3.Напишите запрос, который выполняет выборку имен всех студентов, имеющих по предмету с идентификатором 22 балл ниже общего среднего балла*/
SELECT COUNT(DISTINCT STUDENT_ID), AVG(MARK) FROM EXAM_MARKS GROUP BY MARK HAVING MARK >(SELECT AVG(MARK) FROM EXAM_MARKS WHERE STUDENT_ID=12);
 /*4.Напишите запрос, выполняющий вывод количества предметов, по которым экзаменовался каждый студент, сдававший более 3 предметов*/
